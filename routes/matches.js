...
router.post("/", async (req, res, next) => {
    ...
    if (!user_id) {
        ...
        db.query('SELECT * FROM tbl_users WHERE id=? AND status=1', [user_id], function (err, rows) {
            if (err) {
                db.end();
                message = err;
                status = "error";
            ...
            if (rows?.length > 0) {
                if (rows[0].today_matches_show == 0) {
                    ...
                    // Determine the number of profiles to suggest based on gender
                    var matchCount = rows[0].gender === 'Male' ? 1 : 2;
                    var match_sql1;

                    // Matching profiles based on study, interest, industry, and distance
                    if (rows[0].distance_prefrences > 0) {
                        match_sql1 = `SELECT tmp.* FROM(SELECT *,TIMESTAMPDIFF(YEAR, str_to_date(dob, '%d/%m/%Y'), CURDATE()) AS Age,round(( 6371 * acos( cos( radians(round(${rows[0].latitude},2)) ) * cos( radians( latitude) ) *
                            ...
                            AND tmp.Age >= ${rows[0].age_prefrences_min} AND tmp.Age <= ${rows[0].age_prefrences_max} ORDER BY tmp.distance ASC LIMIT ${matchCount}`;
                    } else {
                        match_sql1 = `SELECT tmp.* FROM (SELECT *,TIMESTAMPDIFF(YEAR, str_to_date(dob, '%d/%m/%Y'), CURDATE()) AS Age,round(( 6371 * acos( cos( radians(round(${rows[0].latitude},2)) ) * cos( radians( latitude) ) *
                            cos( radians( longitude) - radians(round(${rows[0].longitude},2)) ) + sin( radians(round(${rows[0].latitude},2)) ) * sin( radians(latitude) ) ) )) AS distance
                            FROM tbl_users WHERE (study_main_cat='${rows[0].study_main_cat}' OR industry_main_cat='${rows[0].industry_main_cat}' OR interests LIKE '%${rows[0].interests}%')
                            ...
                            ORDER BY tmp.distance ASC LIMIT ${matchCount}`;
                    }
                    ...
                    db.query(match_sql1, function (err, rows1) {
                        ...
                        // For males, if no matches are found, add a random match
                        if (rows[0].gender === 'Male' && first_array.length < 1) {
                            extra_qry = extra_qry.replace(')', `,${first_array.map(u => u.id).join(',')})`);
                            match_sql2 = `SELECT tmp.* FROM(SELECT *,TIMESTAMPDIFF(YEAR, str_to_date(dob, '%d/%m/%Y'), CURDATE()) AS Age,round(( 6371 * acos( cos( radians(round(${rows[0].latitude},2)) ) * cos( radians( latitude) ) *
                                cos( radians( longitude) - radians(round(${rows[0].longitude},2)) ) + sin( radians(round(${rows[0].latitude},2)) ) * sin( radians(latitude) ) ) )) AS distance
                                 FROM tbl_users WHERE status=1 ${genderextra_query} AND is_pause=0 AND id <> 1 AND id <>  ${rows[0].id} ${extra_qry} 
                                 AND id NOT IN (SELECT profile_id FROM tbl_profile_like WHERE user_id=${rows[0].id})
                                 AND id NOT IN (SELECT profile_id FROM tbl_profile_reject WHERE user_id=${rows[0].id}) ) AS tmp
                                 ORDER BY tmp.distance,rand() LIMIT 1`;
                            ...
                            db.query(match_sql2, async (err, rows2) => {
                                if (err) {
                                    ...
                                    if (secondArrResponse) {
                                        matches_user = [...new Set(matches_user)];
                                        if (matches_user.length > 0) {
                                            db.query(`UPDATE tbl_users SET today_matches_show=1 WHERE id=${user_id}`, function (err) {
                                                if (err) {
                                                    db.end();
                                                    message = err;
                                                    status = "error";
                                                    ...
                                                    status = "success";
                                                    message = "Today's matches found successfully..!";
                                                    res.status(200).json({ status: status, message: message, data: [...first_array, ...second_array] });
                                                }
                                            });
                                        } else {
                                            status = "success";
                                            message = "No matches found..!";
                                            res.status(200).json({ status: status, message: message, data: [] });
                                        }
                                    }
                                }
                            });
                        } else if (rows[0].gender === 'Female') {
                            // For females, if less than 2 matches are found, add random matches to make it 2
                            let remainingCount = 2 - first_array.length;
                            if (remainingCount > 0) {
                                extra_qry = extra_qry.replace(')', `,${first_array.map(u => u.id).join(',')})`);
                                match_sql2 = `SELECT tmp.* FROM(SELECT *,TIMESTAMPDIFF(YEAR, str_to_date(dob, '%d/%m/%Y'), CURDATE()) AS Age,round(( 6371 * acos( cos( radians(round(${rows[0].latitude},2)) ) * cos( radians( latitude) ) *
                                    cos( radians( longitude) - radians(round(${rows[0].longitude},2)) ) + sin( radians(round(${rows[0].latitude},2)) ) * sin( radians(latitude) ) ) )) AS distance
                                     ...
                                     AND id NOT IN (SELECT profile_id FROM tbl_profile_like WHERE user_id=${rows[0].id})
                                     AND id NOT IN (SELECT profile_id FROM tbl_profile_reject WHERE user_id=${rows[0].id}) ) AS tmp
                                     ORDER BY tmp.distance,rand() LIMIT ${remainingCount}`;
                                ...
                                if (matches_user.length > 0) {
                                    db.query(`UPDATE tbl_users SET today_matches_show=1 WHERE id=${user_id}`, function (err) {
                                        if (err) {
                                            ...
                                            status = "success";
                                            message = "Today's matches found successfully..!";
                                            res.status(200).json({ status: status, message: message, data: first_array });
                                        }
                                    });
                                } else {
                                    status = "success";
                    ...
                    status = "success";
                    message = "Today's matches already shown..!";
                    res.status(200).json({ status: status, message: message, data: [] });
                }
            } else {
                status = "error";
